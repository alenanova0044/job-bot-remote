...
- hard_prefilter() — дешёвый, бесплатный отсев ДО обращения к Gemini
  (стоп-компании, стоп-роли, антиспам, жёсткий remote-фильтр с исключением
  "гибрид, но Сербия").
- score_vacancy() — полный rule-based скоринг, запасной вариант, если
  Gemini недоступен (нет ключа, кончился лимит, сбой сети).

Единственный критерий занятости — формат работы (remote). Part-time/full-time
не влияют на скор. Индустрия сама по себе никогда не блокирует вакансию —
только даёт бонус, если совпала с priority_industry_keywords.
"""
import re
...
def _stem_pattern(phrase: str) -> re.Pattern:
    """Строит regex по корням слов — чтобы 'проектный менеджер' находил и
    'проектного менеджера', 'проектному менеджеру' и т.д."""
    words = phrase.split()
    parts = []
    for w in words:
...
def hard_prefilter(vac: dict, criteria: dict) -> tuple[bool, str]:
    ...
    Проверяет: бренды из чёрного списка, стоп-роли, антиспам, жёсткий remote-фильтр."""
    text = vac.get("raw_text", "") or (vac.get("title", "") + " " + vac.get("snippet", ""))

    if _contains_any(text, criteria.get("stop_companies", [])):
        return False, "компания из чёрного списка (маркетплейсы-гиганты)"
    if _contains_any(vac.get("title", ""), criteria["stop_roles"]):
        return False, "стоп-роль (не операционное направление / не подходящий тип работы)"
...
def score_vacancy(vac: dict, criteria: dict) -> tuple[int, list[str]]:
    """Rule-based скоринг — используется как запасной вариант, если Gemini
    недоступен (нет ключа, кончился лимит, сбой сети)."""
    text = vac.get("raw_text", "") or (vac.get("title", "") + " " + vac.get("snippet", ""))
    reasons = []
    score = 0
    ...
    if _contains_any(text, criteria["priority_industry_keywords"]):
        score += 2
        reasons.append("приоритетная индустрия/продукт (health/edtech/психология/НКО/SaaS и т.п.)")
    ...
    if numbers:
        ...
        reasons.append("зарплата не указана (не штраф)")
